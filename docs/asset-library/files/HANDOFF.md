# Panic Salon — Demo 34 asset handoff

## Current appearance
Demo 34 uses procedural Three.js characters from granny-model.mjs, girl-model.mjs and man-model.mjs. Edit these or use them as the visual reference. The Blender bundles are separate rigged studies for colleagues to refine; they are not silently substituted into the demo.

## Try the source
Serve the `site` directory with a local static server. Camera access requires HTTPS or localhost. No npm build is required. `game.js` owns service flow and cue timing; `avatar.mjs` owns 3D tools, scene props and result effects.
...
## UI
`index.html` and `style.css` contain HUD, tap/swipe guides, buttons and confetti. The guides last two seconds in play. SVGs are extracted originals. `payment.mjs` paints the banknote in a canvas; it is game currency.
